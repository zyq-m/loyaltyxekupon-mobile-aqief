...
const B40CollectPoint = () => {
  ...
  const onNextButtonPress = () => {
    if (active.btn1) {
      navigation.navigate("B40CashlessCampaign"); // Navigate to the "CashlessScreen" when btn1 is active
    } else if (active.btn2) {
      navigation.navigate("B40GreenCampusCampaign"); // Navigate to the "GreenCampusScreen" when btn2 is active
    }
  };
  return (
    <View style={[globals.container, { paddingHorizontal: 16 }]}>
      <View style={{ flex: 1, justifyContent: "center" }}>
        <Text style={[CPStyle.textCenter, CPStyle.payHeader]}>
          Choose campaign to collect point
        </Text>
...
export default B40CollectPoint;
